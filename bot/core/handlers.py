...
# Import all modules including the resume_tasks function
from ..modules import *
from ..modules.anonymous import verify_anno
from ..modules.gensession import handle_command, handle_cancel_button, gen_session
from ..modules.direct_selector import register_direct_selector_handlers
...
def add_handlers():
    ...
    # TgClient.bot.add_handler(CallbackQueryHandler(start_cb, filters=regex("^start"))) # Removed old verification callback
    TgClient.bot.add_handler(
        MessageHandler(
            torrent_search,
    ...
    TgClient.bot.add_handler(
        CallbackQueryHandler(verify_anno, filters=regex("^verify"))
    )
    TgClient.bot.add_handler(
        MessageHandler(
            get_users_settings,
    ...
    TgClient.bot.add_handler(
        CallbackQueryHandler(handle_cancel_button, filters=regex("^gensession_cancel"))
    )
    TgClient.bot.add_handler(
        CallbackQueryHandler(gen_session, filters=regex("^gensession$"))
    )
    TgClient.bot.add_handler(
        MessageHandler(
            handle_command,
    ...
    # Register direct selector handlers
    register_direct_selector_handlers(TgClient.bot)
